using System;
using System.Globalization;
using uParser.Exceptions;

namespace uParser
{
    public static partial class UnityParser
    {
        private const string k_HexColorRegxPattern = @"^#(?:[0-9a-fA-F]{3}){1,2}$";
        private const string k_PathPrefix = "p:";

        private static readonly char[] k_TrimChars = new char[]
        {
            '(',
            ')',
        };

        private static readonly string[] k_SeparateStrings = new string[]
        {
            ",",
            "'",
        };

        private static ArgumentType[] GenericParser<ArgumentType, ObjectType>(
            string input, int minRange, int maxRange, string[] separators = null)
            where ArgumentType : IConvertible
        {
            if (input is null)
                throw new ArgumentNullException(nameof(input));

            string[] array = GenericSplitter<ObjectType>(input, minRange, maxRange, separators);
            ArgumentType[] values = new ArgumentType[array.Length];

            for (int i = 0; i < array.Length; i++)
            {
                if (array[i].Trim() == null || DBNull.Value.Equals(array[i].Trim()))
                    continue;

                try
                {
                    values[i] = (ArgumentType)Convert.ChangeType(
                        array[i].Trim(), typeof(ArgumentType), CultureInfo.InvariantCulture);
                }
                catch (FormatException)
                {
                    //if (TryParse(array[i].Trim(), out var obj))
                    //{
                    //    values[i] = (ArgumentType)Convert.ChangeType(obj, typeof(ArgumentType), CultureInfo.InvariantCulture);
                    //}
                    //else
                    //    values[i] = default(ArgumentType);

                    values[i] = default(ArgumentType);
                }
            }

            return values;
        }

        private static string[] GenericSplitter<T>(
            string input, int minRange, int maxRange, string[] separators = null)
        {
            if (separators == null)
                separators = k_SeparateStrings;

            if (input is null)
                throw new ArgumentNullException(nameof(input));

            string[] array = input.Split(separators, StringSplitOptions.RemoveEmptyEntries);

            if (array.Length < minRange || array.Length > maxRange)
                throw new InvalidArgumentFormat(input, typeof(T));

            return array;
        }
    }
}
